...
func (b *Button) Help() string {
	...
	if b.ActiveLow {
		help.WriteString(fmt.Sprintf("The button is active low. Thus 'Idle' mean 'High', 'Pressed' means 'Low'\n"))
	} else {
		help.WriteString(fmt.Sprintf("The button is active high. Thus 'Idle' mean 'Low', 'Pressed' means 'High'\n"))
	}
