...
type session struct {
	...
	// BidiStream forbids concurrent Send calls, and both workers (toClientWorker
	// and fromClientWorker) send responses, so every send must hold this lock.
	sendMu sync.Mutex
...
// the client disconnected mid-transfer) so a half-finished transfer cannot
// wedge WaitForTransfers — and therefore graceful shutdown — forever.
