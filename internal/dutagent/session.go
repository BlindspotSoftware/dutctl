...
// receiveFileChunkData handles receiving a file chunk from the client using the new protocol.
// It writes chunks directly to a temporary file to avoid channel blocking issues.
// Note: This is called from fromClientWorker when FileTransferChunk messages are received.
func (s *session) receiveFileChunkData(data []byte, offset uint64, isLast bool) error {
